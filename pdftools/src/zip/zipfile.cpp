...
void ZipFile::add_source(const char *filename, const char *buffer, int length)
{
    ...
    file.length = length;
    file.crc = crc32((uint8_t *) buffer, length);
...
uint32_t ZipFile::crc32(uint8_t byte, uint32_t crc32)
{
    return ((crc32) >> 8) ^ crc_table[(byte) ^ ((crc32) & 0x000000FF)];
}

uint32_t ZipFile::crc32(uint8_t *data, uint32_t size)
{
    uint32_t loop;
    uint32_t crc = 0xFFFFFFFF;

    for (loop = 0; loop < size; loop++) {
        crc = crc32((char) *data, crc);
        data++;
    }
    crc = ~crc;
    return crc;
